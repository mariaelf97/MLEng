...
select batter,year(local_date), Hit/nullif(atBat,0) as Average_at_bat from joined_batter_game group by batter,year(local_date) order by year(local_date);
...
select batter, Hit/nullif(atBat,0) as Average_at_bat from joined_batter_game group by batter;
...
select batter, Hit/nullif(atBat,0) as batting_average from last_100_days_per_player group by batter;
